...
<script>
  ...
  export default {
    ...
    mounted() {
      let _this = this;
      //_this.$route.query.id; 获得路由目标路径后面根的查询语句 条件参数
    ...
    methods: {
      findCourse() {
        let _this = this;
        _this.$ajax.get('/business/web/course/find/' + _this.id).then((response)=>{
      ...
      doFolded (chapter, i) {
        ...
        //$set 是强制更改vue data中的数据 vue可以监听到并且响应
        //这个只有在操作数组的时候 会用到
      ...
      play(section) {
        ...
        // _this.$refs.modalPlayer.playVod(section.video);
        //播放没有加密的mp4连接
      ...
      enroll() {
        ...
        _this.$ajax.post('/business/web/member-course/enroll', {
      ...
      getEnroll() {
        ...
        _this.$ajax.post('/business/web/member-course/get-enroll', {
          courseId: _this.course.id,
          memberId: loginMember.id
        }).then((response)=>{
          let resp = response.data;
