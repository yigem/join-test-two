                    ...
                    <div class="modal-body">
                        <form class="form-horizontal">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-10">
                                                <input v-model="role.name" class="form-control" placeholder="角色,无错类型:String">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-2 control-label">描述</label>
                                            <div class="col-sm-10">
                                                <input v-model="role.desc" class="form-control" placeholder="描述,无错类型:String">
                                            </div>
                                        </div>
                        </form>
                    </div>
...
export default {
    ...
    methods:{
        ...
        add(){
            let _this = this;
            _this.role = {};//新增时 清空文本框中内容
            $("#form-modal").modal("show");
        },//end method add
        list(page){
            Loading.show();
            let _this = this;
            let url = "/system/admin/role/list";
            _this.$ajax.get(url,{
                params:{
                    page:encodeURI(page),
                ...
                (response)=>{
                    ...
                    //给双向绑定数据赋值
                    _this.roles = result.content.list;
                    //获得数据后渲染分页组件数据
       ...
       save(){
           ...
           if (1 != 1
            ...
            Loading.show();
            let url = "/system/admin/role/save";
            _this.$ajax.post(url,_this.role,
            ).then(
                (response)=>{
                    Loading.hide();
        ...
        del(id){
            ...
            Confirm.show("删除后不可恢复,请三思而后行",function(){
                Loading.show();
                let url = "/system/admin/role/delete/";
      ...
      loadResource() {
        let _this = this;
        Loading.show();
        _this.$ajax.get('/system/admin/resource/load-tree').then((res)=>{
          Loading.hide();
       ...
       */
       saveResource() {
        ...
        _this.$ajax.post('/system/admin/role/save-resource', {
          id: _this.role.id,
       ...
       */
       listRoleResource() {
        let _this = this;
        _this.$ajax.get('/system/admin/role/list-resource/' + _this.role.id).then((response)=>{
          let resp = response.data;
       ...
       */
       listUser() {
        let _this = this;
        _this.$ajax.post('/system/admin/user/list', {
          page: 1,
          size: 9999
        }).then((response)=>{
          let resp = response.data;
       ...
       */
       saveUser() {
        ...
        _this.$ajax.post('/system/admin/role/save-user', {
          id: _this.role.id,
          userIds: userIds
        }).then((response)=>{
      ...
      listRoleUser() {
        let _this = this;
        _this.roleUsers = [];
        _this.$ajax.get('/system/admin/role/list-user/' + _this.role.id).then((res)=>{
          let response = res.data;
